...
class ContentSettings(BaseModel):
    ...
    local_cache_root: Path = Path("./runtime/content-cache")
    cache_max_gb: float = 8.0

    @field_validator("backend")
    @classmethod
    def normalise_backend(cls, value: str) -> str:
        backend = value.strip().lower()
        if backend not in {"filesystem", "s3"}:
            raise ValueError("content.backend must be 'filesystem' or 's3'")
        return backend

    @model_validator(mode="after")
    def validate_backend_settings(self) -> "ContentSettings":
        if self.backend == "s3" and not self.s3_bucket:
            raise ValueError("content.s3_bucket is required when content.backend is 's3'")
        if self.cache_max_gb <= 0:
            raise ValueError("content.cache_max_gb must be greater than 0")
        return self
...
class RadioSettings(BaseModel):
    ...
    def ensure_runtime_paths(self) -> None:
        ...
        self.content.local_cache_root.mkdir(parents=True, exist_ok=True)
...
def _apply_environment_overrides(payload: dict) -> dict:
    """Overlay RADIO__SECTION__FIELD env vars onto the YAML configuration."""
    overrides: dict[str, object] = {}
    for key, raw_value in os.environ.items():
        if not key.startswith("RADIO__"):
            continue

        path = [segment.strip().lower() for segment in key.split("__")[1:] if segment.strip()]
        if not path:
            continue
    ...
    return merged if isinstance(merged, dict) else payload
...
def _parse_env_value(raw_value: str) -> object:
    if raw_value == "":
        return ""
